---
id: d2ee65fc-7a67-40c4-9300-1697c929ff99
origin: c1a2b3d4-0000-4e5f-8a9b-0c1d2e3f4a04
title: Service
text: 'Du premier conseil jusque longtemps après la pose, vous pouvez compter sur nous, pendant toute la durée de vie de votre installation.'
meta_title: 'Service et entretien'
meta_description: 'Conseils, prise de mesures, pose par notre propre équipe, contrôle périodique et garantie. Un interlocuteur fixe près de chez vous, même des années après la pose.'
services:
  -
    id: serviceadvies
    type: service
    overline: Conseil
    title: 'À la mesure de votre habitation'
    text:
      -
        type: paragraph
        attrs:
          textAlign: left
        content:
          -
            type: text
            text: 'Avant toute décision, nous réfléchissons avec vous. Nous passons gratuitement chez vous, prenons toutes les mesures avec précision et examinons votre habitation sur place. Sur cette base, nous proposons la solution qui correspond à votre situation, vos goûts et votre budget. Vous recevez des conseils honnêtes et un devis clair, sans surprises par la suite.'
    image: service/winsol-2.jpg
    enabled: true
  -
    id: serviceinstallatie
    type: service
    overline: Installation
    title: 'Posé dans les règles de l''art'
    text:
      -
        type: paragraph
        attrs:
          textAlign: left
        content:
          -
            type: text
            text: 'Votre installation est posée par notre propre équipe, sans sous-traitants. Nos spécialistes connaissent leur métier et traitent votre habitation avec respect. Nous travaillons proprement et rangeons tout après la pose. À la réception, nous vous montrons comment tout fonctionne, pour que vous puissiez en profiter immédiatement l''esprit tranquille.'
    image: service/winsol-2_1.jpg
    enabled: true
  -
    id: serviceonderhoud
    type: service
    overline: Entretien
    title: 'Contrôle sur rendez-vous'
    text:
      -
        type: paragraph
        attrs:
          textAlign: left
        content:
          -
            type: text
            text: 'Une installation bien entretenue dure des années de plus. Nous passons sur rendez-vous pour un contrôle périodique, le réglage et la lubrification des pièces mobiles. Les petites réparations, nous les faisons directement, avant qu''elles ne deviennent de gros problèmes. Ainsi, tout continue à fonctionner sans accroc et en toute sécurité, saison après saison. Et en cas de souci, nous sommes rapidement sur place.'
    image: service/winsol-2_2.jpg
    enabled: true
  -
    id: servicegarantie
    type: service
    overline: Garantie
    title: 'Un suivi qui dure'
    text:
      -
        type: paragraph
        attrs:
          textAlign: left
        content:
          -
            type: text
            text: 'Chaque installation bénéficie à la fois de la garantie d''usine et de la garantie de pose. Et même après la pose, nous restons tout simplement joignables. Vous avez un seul interlocuteur fixe près de chez vous, même des années plus tard. Une question ou un problème ? Nous assurons un suivi rapide. Vous n''êtes ainsi jamais seul avec votre achat.'
    image: service/winsol-2_3.jpg
    enabled: true
reparation:
  overline: Réparation
  title: 'Quelque chose de cassé ou qui ne fonctionne plus ?'
  text: 'Pour les clients existants qui rencontrent un problème. Décrivez brièvement ce qui se passe et nous vous contactons pour passer chez vous.'
  image: quicklinks/herstelling.png
---
