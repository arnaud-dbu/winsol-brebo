---
id: 9b4f52b1-0003-4c2d-8e6f-4a5b6c7d9e03
title: 'PVC ramen'
range:
  - 8c2e41a0-0002-4a1b-9c7d-3e5f6a7b8c02
text: 'Uitstekend isolerende ramen die nauwelijks onderhoud vragen, in een ruim aanbod kleuren en houtlook-afwerkingen.'
image: ramen-en-deuren/realisatie-realisation_pvc_c-70_beauraing-(20).jpg
brochure: brochures/winsol_brochure_ramen-en-deuren-in-pvc_nl.pdf
meta_title: 'PVC ramen op maat | Winsol Dilbeek, Sint-Pieters-Leeuw & Aartselaar'
meta_description: 'PVC ramen van Winsol, reeks C+70: een isolatiewaarde tot 1,1 W/m²K, inbraakwerendheid tot RC2 en nauwelijks onderhoud. Op maat geplaatst.'
seo_noindex: false
page_builder:
  -
    id: pvr-isolatie
    type: text_image
    overline: Isolatie
    title: 'Warmte binnen, lawaai buiten'
    text:
      -
        type: paragraph
        attrs:
          textAlign: left
        content:
          -
            type: text
            text: 'De C+70-reeks haalt een isolatiewaarde tot 1,1 W/m²K. Dat merk je aan je energiefactuur, en aan hoe stil het binnen wordt als je aan een drukke straat woont.'
      -
        type: paragraph
        attrs:
          textAlign: left
        content:
          -
            type: text
            text: 'Pvc vraagt bovendien nauwelijks onderhoud: afnemen met water volstaat, schilderen is nooit nodig.'
    media: image
    image: ramen-en-deuren/realisatie-realisation-pvc-c-70-aalbeke-(21).jpg
    features:
      -
        id: pvr-f1
        type: feature
        label: 'Isolatiewaarde tot 1,1 W/m²K'
        enabled: true
      -
        id: pvr-f2
        type: feature
        label: 'Nauwelijks onderhoud'
        enabled: true
      -
        id: pvr-f3
        type: feature
        label: 'Ruim aanbod kleuren en houtlook'
        enabled: true
    background: false
    enabled: true
  -
    id: pvr-specs
    type: technical_details
    overline: Technisch
    title: 'De cijfers'
    text: 'Uit de productbrochure. Wat in jouw situatie haalbaar is, bepalen we bij het opmeten.'
    technical_details:
      -
        id: pvr-s1
        key: Materiaal
        value: 'Pvc, reeks C+70'
      -
        id: pvr-s2
        key: Isolatiewaarde
        value: 'Tot 1,1 W/m²K'
      -
        id: pvr-s3
        key: Profieldiepte
        value: '70 mm'
      -
        id: pvr-s4
        key: Inbraakwerendheid
        value: 'Klasse RC1 of RC2'
      -
        id: pvr-s5
        key: Lucht- en waterdichtheid
        value: 'Klasse 4'
      -
        id: pvr-s6
        key: Onderhoud
        value: 'Afnemen met water, nooit schilderen'
    enabled: true
  -
    id: pvr-galerij
    type: image_gallery
    overline: Realisaties
    title: 'PVC ramen in de praktijk'
    images:
      - ramen-en-deuren/realisatie-realisation_pvc_c-70_beauraing-(16).jpg
      - ramen-en-deuren/realisatie-realisation-pvc-c-70-aalbeke-(17).jpg
      - ramen-en-deuren/realisatie-realisation_pvc_c-70_zottegem-(4).jpg
      - ramen-en-deuren/realisatie-realisation-pvc-ramen-c-70-pvc-fenetres-c-70_gent_05.jpg
      - ramen-en-deuren/realisatie-realisation_pvc_c-70_beauraing-(57).jpg
      - ramen-en-deuren/realisatie-realisation_pvc_c-70_zottegem-(37).jpg
    enabled: true
---
