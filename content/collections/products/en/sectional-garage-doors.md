---
id: c5ec2e3b-d1ef-4268-b958-680f1df89480
origin: 9d6f52b1-0001-4c2d-8e6f-4a5b6c7db001
title: 'Sectional garage doors'
text: 'Opens vertically, so you lose no space in the garage and can park right up against the door.'
meta_title: 'Sectional garage doors'
meta_description: 'Sectional garage doors from Winsol in the Isol-Comfort, Comfort and Comfort-Go ranges. Double-walled panels, RC2 certification and fully made to measure.'
page_builder:
  -
    id: sp-ruimte
    type: text_image
    overline: 'How it works'
    title: 'The door disappears beneath the ceiling'
    text:
      -
        type: paragraph
        attrs:
          textAlign: left
        content:
          -
            type: text
            text: 'A sectional door consists of horizontal panels that hinge upwards as it opens and run along the garage ceiling. Nothing swings outwards, which means your car can sit right up against the door. Inside, the full driving width and almost the entire floor area remain usable.'
      -
        type: paragraph
        attrs:
          textAlign: left
        content:
          -
            type: text
            text: 'Operation is electric, with a remote control from the car. That is also exactly why this type has become all but the standard today.'
    media: image
    image: garagepoorten/OneDrive_1_31-07-2026/garagepoort-porte-de-garage-2.jpg
    features:
      -
        id: sp-r1
        type: feature
        label: 'No loss of space inside'
        enabled: true
      -
        id: sp-r2
        type: feature
        label: 'Park right up against the door'
        enabled: true
      -
        id: sp-r3
        type: feature
        label: 'Electric with remote control'
        enabled: true
    background: false
    enabled: true
  -
    id: sp-reeksen
    type: text_image
    overline: Ranges
    title: 'Three ranges, for three different needs'
    text:
      -
        type: paragraph
        attrs:
          textAlign: left
        content:
          -
            type: text
            text: 'Isol-Comfort is the range for an integral garage. It borders directly on the house, so the door plays its part in whether you meet the insulation standards. Thicker double-walled panels and excellent airtightness make the difference there.'
      -
        type: paragraph
        attrs:
          textAlign: left
        content:
          -
            type: text
            text: 'Comfort is the broad range: every size, every finish, including a rustic country-house finish, so the door matches your windows and doors. Comfort-Go is its compact sibling, made to replace an old up-and-over door, with faster installation and maximum dimensions of 3250 by 2235 mm.'
    media: image
    image: garagepoorten/2019_Tielt/garagepoorten-portes-de-garage_productnaam-(10).jpg
    features:
      -
        id: sp-x1
        type: feature
        label: 'Isol-Comfort for integral garages'
        enabled: true
      -
        id: sp-x2
        type: feature
        label: 'Comfort in every size and finish'
        enabled: true
      -
        id: sp-x3
        type: feature
        label: 'Comfort-Go replaces an up-and-over door'
        enabled: true
    background: true
    enabled: false
  -
    id: sp-specs
    type: technical_details
    overline: Technical
    title: 'The figures'
    text: 'We determine what fits your opening when we come to measure up.'
    technical_details:
      -
        id: sp-s1
        key: Ranges
        value: 'Isol-Comfort, Comfort and Comfort-Go'
      -
        id: sp-s2
        key: Panels
        value: 'Double-walled, in hot-dip galvanised steel with a twice-baked polyester coating'
      -
        id: sp-s3
        key: 'Burglar resistance'
        value: 'RC2 security certificate'
      -
        id: sp-s4
        key: Operation
        value: 'Electric, with remote control'
      -
        id: sp-s5
        key: 'Comfort-Go maximum'
        value: '3250 x 2235 mm'
      -
        id: sp-s6
        key: Finish
        value: 'Every size and colour, including a rustic country-house finish'
    enabled: true
  -
    id: sp-galerij
    type: image_gallery
    overline: Projects
    title: 'Sectional doors in practice'
    images:
      - garagepoorten/OneDrive_1_31-07-2026/garagepoort-porte-de-garage-1.jpg
      - garagepoorten/OneDrive_1_31-07-2026/garagepoort-porte-de-garage-2.jpg
      - garagepoorten/OneDrive_1_31-07-2026/garagepoort-porte-de-garage-3.jpg
      - garagepoorten/OneDrive_1_31-07-2026/garagepoort-porte-de-garage-4.jpg
      - garagepoorten/OneDrive_1_31-07-2026/garagepoort-porte-de-garage-5.jpg
      - garagepoorten/OneDrive_1_31-07-2026/garagepoort-porte-de-garage-6.jpg
      - garagepoorten/OneDrive_1_31-07-2026/garagepoort-porte-de-garage-7.jpg
      - garagepoorten/OneDrive_1_31-07-2026/garagepoort-porte-de-garage-8.jpg
      - garagepoorten/OneDrive_1_31-07-2026/garagepoort-porte-de-garage-9.jpg
      - garagepoorten/OneDrive_1_31-07-2026/realisatie-realisation-garagepoorten-portes-de-garage-(17).jpg
      - garagepoorten/OneDrive_1_31-07-2026/realisatie-realisation-garagepoorten-portes-de-garage-(18).jpg
      - garagepoorten/OneDrive_1_31-07-2026/realisatie-realisation-garagepoorten-portes-de-garage-(19).jpg
      - garagepoorten/OneDrive_1_31-07-2026/realisatie-realisation-garagepoorten-portes-de-garage-(20).jpg
      - garagepoorten/OneDrive_1_31-07-2026/realisatie-realisation-garagepoorten-portes-de-garage-(21).jpg
      - garagepoorten/OneDrive_1_31-07-2026/realisatie-realisation-garagepoorten-portes-de-garage-(22).jpg
      - garagepoorten/OneDrive_1_31-07-2026/realisatie-realisation-garagepoorten-portes-de-garage-(23).jpg
      - garagepoorten/OneDrive_1_31-07-2026/realisatie-realisation-garagepoorten-portes-de-garage-(24).jpg
      - garagepoorten/OneDrive_1_31-07-2026/realisatie-realisation-garagepoorten-portes-de-garage-(25).jpg
      - garagepoorten/OneDrive_1_31-07-2026/realisatie-realisation-garagepoorten-portes-de-garage-(26).jpg
    enabled: true
---
