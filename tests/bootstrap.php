 ...
 *
 * Met `CACHE_STORE=file` (zie phpunit.xml) overleeft zo'n corrupte lijst de
 * hele testrun — en zonder expliciete opruiming ook een volgende run, met
 * een groene suite op verouderde data als risico. Daarom hier, vóór er één
 * test draait, in een apart proces: cache wissen, dan de Stache warmen. Zo
 * gebeurt de onvermijdelijke koude opbouw eenmalig en gecontroleerd, in
 * plaats van willekeurig middenin een test.
 */
...
exec('find '.escapeshellarg($cacheDir).' -mindepth 1 ! -name .gitignore -delete');

exec(
    'cd '.escapeshellarg($root).' && CACHE_STORE=file APP_ENV=testing php artisan statamic:stache:warm --no-interaction 2>&1',
    $output,
    $exitCode
);

if ($exitCode !== 0) {
    fwrite(STDERR, "Waarschuwing: Stache-warmup vóór de testrun is mislukt (exit {$exitCode}):\n".implode("\n", $output)."\n");
}
